package nfort.com.oop.lesson26.task;
...
public class Task2 {
    ...
    private static int trackTime(Supplier<Integer> task) {
        ...
        int result = task.get();
        System.out.println(result + ": " + (System.currentTimeMillis() - starTime));
        return result;
    }
...
//        return IntStream.of(values)
//                .parallel()         // делится на то количество потоков, которое позволяет процессор
//                .max()
//                .orElse(Integer.MIN_VALUE);
//    }
}
